...
MsgResult_t UserDataDownload_Ready(UnitPara_t *pData)
{
	...
	UserDownload.frameCount = NUM_ROUND(dataSize,UserDownload.frameSize);//获取数据总包数
	...
	{	
		...
		if((DevPartTab->user_data_start_addr <= UserDownload.addr)&&	//存储地址合法
		   (DevPartTab->flash_end_addr > (dataSize + UserDownload.addr)))//数据存储数据不会溢出	
		{	
			return ((MsgResult_t)(Program_Erase(UserDownload.addr, dataSize)));	//擦除用户Flash区
		}		
	}
...
MsgResult_t UserDataFrame_Save(UnitPara_t *pData)
{
	...
	if (!(UserDownload.frameFlag[id>>5]&(1<<(id&0x1f))))
	{
	    ...
	    if (TRUE != UserDownload.save(addr, pData->len, (u8*)&pData->data[2]))	//存储失败
	    {
			return CMD_EXE_ERR;
	    }
